    ...
    >
      ...
      <div className="px-4 py-3 flex flex-col gap-2 mt-8 text-foreground">
        <button
          ...
          className="cursor-pointer flex-1 text-sm bg-transparent hover:bg-gray-200/30 rounded-full px-3 py-2 font-bold w-full items-start flex"
        >
        ...
        <button
          ...
          className="cursor-pointer flex-1 text-sm bg-transparent hover:bg-gray-200/30 rounded-full px-3 py-2 font-bold items-start flex w-full"
        >
      ...
      <div className="px-4 py-3">
        <p className="text-sm font-semibold text-primary mb-3 mt-3">
          Riwayat chat
        </p>
        <div className="max-h-64 overflow-y-auto -mx-2 space-y-1">
          {sessions.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`flex items-center cursor-pointer gap-2 px-2 py-1 rounded-lg ${
                  isActive
                    ? "bg-indigo-100 text-indigo-700"
                    : "hover:bg-gray-100/30 text-foreground"
                }`}
              >
                <button
                  onClick={() => handleSessionClick(item.id)}
                  className="flex-1 text-left text-xs cursor-pointer line-clamp-2"
                >
                  <span className="line-clamp-2 cursor-pointer">{label}</span>
                </button>
                <button
                  onClick={() => onDeleteSession(item.id)}
                  className="text-xs cursor-pointer text-destructive hover:text-red-800 transform  px-1 py-0.5 rounded"
                >
          ...
          {sessions.length === 0 && (
            <p className="text-xs text-foreground px-2 py-1">
      ...
      <div className="px-4 py-4 flex items-center justify-between mt-auto">
        <div className="flex items-center gap-3">
          <img src={Logo} alt="pp" className="w-10 h-10 rounded-full border border-white" />
        ...
        <button
          onClick={onLogout}
          className="text-xs cursor-pointer text-destructive px-2 py-1 rounded-full hover:bg-red-50/30 transition-colors"
        >
          <LogOut/>
        </button>
      </div>
